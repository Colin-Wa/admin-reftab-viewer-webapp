...
const SCOPES = ['https://www.googleapis.com/auth/admin.directory.device.chromeos', 'https://www.googleapis.com/auth/admin.directory.user'];
...
passport.use(new GoogleStrategy({
  ...
  db.get('SELECT * FROM federated_credentials WHERE provider = ? AND subject = ?', [
    ...
    if (err) { 
      return cb(err); }
    if (process.env.WHITELIST_MEMBERS.split(',').indexOf(profile.emails[0].value) < 0) {
      console.log(profile.emails[0].value + " not on whitelist");
      err = {}
      err.status = "You are not allowed to access this application"
      return cb(err);
    }
    ...
    if (!row) {
      db.run('INSERT INTO users (name) VALUES (?)', [
        ...
        db.run('INSERT INTO federated_credentials (user_id, provider, subject) VALUES (?, ?, ?)', [
          ...
          return ad_shared_function(user, cb);
        });
      });
    } else {
      db.get('SELECT * FROM users WHERE id = ?', [ row.user_id ], function(err, row) {
        ...
        return ad_shared_function(row,cb);
      });
    }
  });
}));

function ad_shared_function(user, cb)
{
  admin_actions.ad_authorize(user.id);
  return cb(null, user);
}
